...
class Loggingyaml(object):
    ...
    default_folder=r'./',):
        self.default_log_folder=r'./log'
        self.default_path=default_folder+default_name

    def logdirectory(self):
        isExists=os.path.exists(self.default_log_folder)
        if not isExists:
            try:
                os.mkdir(self.default_log_folder)
                self.loggerin().info('已创建日志目录')
            except BaseException as be:
                self.loggerin().error(be)
